...
"""
#def trainModelsBoot(epochs,hiddenLayers,isLinear):
# imports
import torch
import pickle
import glob
...
# load in torch datasets 
filePaths = sorted(glob.glob('../../../data/torchFilesBoot/*.pkl'))
...
for filePath in filePaths:
    ...
    nNeurons = data['trainData'].tensors[0].shape[1]
    ...
    # save model
    outFile = f"../output/sessionsBoot/{file[0:-4]}_hl_{'_'.join(str(i) for i in hiddenLayers)}_epochs_{str(epochs)}_lr_{str(learnRate)}_l2_{str(l2)}_isLin_{str(isLinear)}.pt"
    if saveData:
